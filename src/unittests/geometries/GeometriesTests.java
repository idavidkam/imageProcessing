...
public class GeometriesTests {
	...
	public void testFindIntersections() {
		...
		var geometroies = new Geometries(new Plane(new Point3D(-4, 0, 0), new Point3D(0, 0, 2), new Point3D(0, 0, 0)),
		...
		assertEquals("Only one shape is cut", 3,
				geometroies.findIntersections(new Ray(new Point3D(0.5, -1, 0.5), new Vector(0, 1, 0))).size());

		// =============== Boundary Values Tests ==================
		Geometries geometroiesEmpty = new Geometries();
		// TC02: Empty body collection
		assertNull("is Empty!",
				geometroiesEmpty.findIntersections(new Ray(new Point3D(2, 4, -3), new Vector(-1, 0, 1))));
		...
		assertEquals("Only one shape is cut", 2,
				geometroies.findIntersections(new Ray(new Point3D(0, 0.5, 0), new Vector(0, 1, 0))).size());
		// TC05: All shapes are cut
		assertEquals("Only one shape is cut", 4,
				geometroies.findIntersections(new Ray(new Point3D(0.5, 6, 0.5), new Vector(0, -1, 0))).size());
	}
}